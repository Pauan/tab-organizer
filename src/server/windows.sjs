...
function unfocus_tab(window, tab_id) {
  if (window.has("focused-tab") && window.get("focused-tab") === tab_id) {
    return window.remove("focused-tab");
  } else {
    return window;
  }
}
...
function remove_tab_from_window(state, window_id, tab_id) {
  ...
  state = state.modify("window-ids", function (ids) {
    ...
    window = unfocus_tab(window, tab_id);
...
function remove_tab(state, id) {
  var unloaded = state.get("unloaded-tabs");

  if (unloaded.has(id)) {
    state = state.set("unloaded-tabs", unloaded.remove(id));

  } else {
    var tab = state.get("tab-ids").get(id);
...
function unload_tab(state, id) {
  ...
  var window_id = state.get("tab-ids").get(id).get("window");

  state = state.modify("window-ids", function (ids) {
    return ids.modify(window_id, function (window) {
      return window ..unfocus_tab(id);
    });
  });

  state = state.modify("tab-ids", function (ids) {
    return ids.modify(id, function (tab) {
      return tab.remove("active") ..update_timestamp("unloaded");
    });
  });
...
// TODO
/*function reload_tab(state, id) {
  var old = delayed_events;
  delayed_events = [];
...
function focus_tab(state, id) {
  var tab = state.get("tab-ids").get(id);

  if (tab.has("active")) {
    @session.tab.focus(id);
  } else {
    reload_tab(state, id);
  }
...
var { subscribe, publish } = @Publisher({
  init: function () {
    ...
    var session = @session.subscribe(function (event) {
      // TODO what if this is called before @session.subscribe returns ?
    ...
    window_ids ..@each(function ([id, window]) {
      ...
      // Reset active, and focused tab for unloaded windows
      window_ids = window_ids.set(id, window.remove("active").remove("focused-tab"));
    });
    ...
    var state = @Dict({
      "connections": @connection.subscribe(publish, "server.windows"),
      "unloaded-tabs": @Set(),
      "tab-ids": tab_ids,
...
exports.step = function (state, event) {
  ...
  if (type === @connection.on.connect) {
    ...
    state = state.modify("connections", function (array) {
      return array.insert(id);
    });

    @connection.send(id, @Dict({
      "type": exports.on.init,
      "tab-ids": state.get("tab-ids"),
      "window-ids": state.get("window-ids"),
      "windows": state.get("windows")
    }));

  } else if (type === @connection.on.disconnect) {
    state = state.modify("connections", function (array) {
      return array.remove(array ..@indexOf(event.get("id")));
    });

  } else if (type === @connection.on.send) {
    state = exports.step(state, event.get("value"));

  } else if (type === exports.on.window_open) {
    var window    = event.get("window");
