...
var window_focused_tab = {}    // The tab id which was last focused (per window id)
...
function emit_update_tab(tab) {
  ...
  //@assert.is(old.focused, tab.focused)
  //@assert.is(old.windowId, tab.windowId)

  tabs_by_id ..@set(tab.id, tab)

  if (should_update(old, tab)) {
    var tab_before = @clone(tab)
    // We want every property on `before` to be the same, except for these
    tab_before.url        = old.url
    tab_before.pinned     = old.pinned
    tab_before.title      = old.title
    tab_before.favIconUrl = old.favIconUrl

    emit({
      type: "tabs.update",
      before: tab_created(tab_before),
...
chrome.tabs.onActivated.addListener(function (info) {
  ...
  if (tabs_by_id ..@has(info.tabId)) {
    ...
    var old = window_focused_tab ..@get(info.windowId)
    if (old !== null) {
      o.before = {
    ...
    // TODO setUnique
    @assert.isNot(old, info.tabId)
    window_focused_tab ..@set(info.windowId, info.tabId)
...
chrome.tabs.onRemoved.addListener(function (id, info) {
  ...
  if (tabs_by_id ..@has(id)) {
    ...
    var old = window_focused_tab ..@get(info.windowId)
    if (old !== null && old === id) {
      window_focused_tab ..@set(info.windowId, null)
    }
  ...
  if (windows_isNormal(window)) {
    ...
    if (window.tabs != null) {
      window.tabs ..@each(function (tab) {
        ...
        if (tab.active) {
          window_focused_tab ..@setNew(window.id, tab.id)
        }
      })
